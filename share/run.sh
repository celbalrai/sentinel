...
if [ ! -f /sentinel/bytz.conf ]; then
  if [ -z "$RPCUSER" -o -z "$RPCPASSWORD" -o -z "$RPCPORT" ]; then
    echo "When no /sentinel/bytz.conf is present, you must at least set RPCUSER, RPCPORT and RPCPASSWORD environment variables"
    exit 1
  fi

  echo "" > /sentinel/bytz.conf
  if [ -n "$RPCUSER" ]; then
    echo "rpcuser=${RPCUSER}" >> /sentinel/bytz.conf
  fi
  if [ -n "$RPCPASSWORD" ]; then
    echo "rpcpassword=${RPCPASSWORD}" >> /sentinel/bytz.conf
  fi
  if [ -n "$RPCPORT" ]; then
    echo "rpcport=${RPCPORT}" >> /sentinel/bytz.conf
  fi
fi
...
if [ ! -f /sentinel/sentinel.conf ]; then
  ...
  echo "bytz_conf=/sentinel/bytz.conf" > /sentinel/sentinel.conf
  if [ -n "$RPCHOST" ]; then
