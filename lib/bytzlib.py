...
def is_valid_bytz_address(address, network='mainnet'):
    ...
    bytz_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Bytz address
        return False

    if (address_version != bytz_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('bytz.conf')
