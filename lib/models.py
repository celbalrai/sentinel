...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, bytzd):
        golist = bytzd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_bytzd(bytzd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_bytzd(self, bytzd, rec):
        import decimal
        import bytzlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from bytzd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from bytzd, with every run
        ...
        # get/create, then sync payment amounts, etc. from bytzd - Bytzd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(bytzd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from bytzd! %s" % e)
            govobj.vote_delete(bytzd)
            return (govobj, None)
    ...
    def vote_delete(self, bytzd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(bytzd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, bytzd, signal, outcome):
        import bytzlib
        ...
        output = bytzd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = bytzlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(bytzd, signal)

    def sync_network_vote(self, bytzd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = bytzd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = BYTZD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 bytz addr, non-multisig
            if not bytzlib.is_valid_bytz_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Bytz address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Bytz Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (bytzlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = BYTZD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not bytzlib.is_valid_bytz_address(addr, config.network):
    ...
    def hash(self):
        import bytzlib
        return bytzlib.hashit(self.serialise())
