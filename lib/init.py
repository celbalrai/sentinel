...
def has_bytz_conf():
    ...
    valid_bytz_conf = False

    # ensure bytz_conf exists & readable
    #
    # if not, print a message stating that Bytz Core must be installed and
    # configured, including JSONRPC access in bytz.conf
    try:
        f = io.open(config.bytz_conf)
        valid_bytz_conf = True
    except IOError as e:
        print(e)

    return valid_bytz_conf
...
def main():
    ...
    if not has_bytz_conf():
        print("Bytz must be installed and configured, including JSONRPC access in bytz.conf")
        sys.exit(1)
