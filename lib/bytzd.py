...
class BytzDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some bytzd methods
    ...
    @classmethod
    def from_bytz_conf(self, bytz_dot_conf):
        from bytz_config import BytzConfig
        config_text = BytzConfig.slurp_config_file(bytz_dot_conf)
        creds = BytzConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = bytzlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = bytzlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = bytzlib.blocks_to_seconds(diff)
